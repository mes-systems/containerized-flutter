...
for workflow_name in ci publish; do
  ...
  assert_contains 'builder="${build_ref%%/*}"' "$attest_source"
  assert_contains 'record="${build_ref##*/}"' "$attest_source"
  assert_contains '[[ -n "$builder" && -n "$record" && "$builder" != "$build_ref" ]]' \
    "$attest_source"
  assert_contains '--builder "$builder"' "$attest_source"
  assert_contains 'history inspect "$record"' "$attest_source"
  assert_contains '--format json' "$attest_source"
  assert_no_text_match 'history inspect "$build_ref"' "$attest_source"
